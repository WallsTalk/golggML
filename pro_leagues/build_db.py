...
for league, info in leagues_info.items():
    ...
    for item in league_teams[1:-1]:
        ...
        sqlite_insert_with_param = """INSERT INTO teams (team_id, team_name, region, players) VALUES (?, ?, ?, ?);"""
        data_tuple = (team_object["id"], team_object["acronym"], league, json.dumps(team_object["players"]))
        try:
